...
class AC_Network:
    def __init__(self, state_size, action_size, name, _trainer):
        ...
        self.critic_optim = tf.keras.optimizers.Adam()
...
class Worker:
    def __init__(self, _env, name, s_size, a_size, _trainer, _global_episodes):
        ...
        self.local_AC = AC_Network(s_size, a_size, self.name, _trainer)
    ...
    def work(self, max_steps, gamma, coord):
        ...
        while not coord.should_stop():
            self.local_AC.update_weights()

            policy_grads, critic_grads = self._run_episode(max_eps_steps=max_steps)
            master_network.policy_optim.apply_gradients(zip(policy_grads, master_network.policy.trainable_variables))
            master_network.critic_optim.apply_gradients(zip(critic_grads, master_network.critic.trainable_variables))
...
if __name__ == '__main__':
    ...
    with tf.device("/cpu:0"):
        ...
        trainer = tf.keras.optimizers.Adam(learning_rate=1e-4)
        master_network = AC_Network(state_size=state_shape, action_size=action_shape, name='global', _trainer=trainer)
        num_workers = 4
        workers = []
        for i in range(num_workers):
            workers.append(
                Worker(env, i, state_shape, action_shape, trainer, global_episodes)
            )
